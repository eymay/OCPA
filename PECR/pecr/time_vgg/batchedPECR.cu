...
__global__ void BatchedPECR(int batch_size, float *input, float *kernel, float *output,
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        */
        ...
        cuda_status = cudaMalloc((void **)&gpu_output, gpu_output_size * sizeof(float));
        if (cuda_status != cudaSuccess)
        {
        ...
        // 传回结果
        float *result = new float[gpu_output_size];
        cuda_status = cudaMemcpy(result, gpu_output, gpu_output_size * sizeof(float), cudaMemcpyDeviceToHost);
        if (cuda_status != cudaSuccess)
        {
